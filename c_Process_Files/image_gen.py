...
def array_to_img(x, data_format=None, scale=True):
    ...
    if data_format is None:
        data_format = keras_image_data_format()
...
class ImageDataGenerator(object):
    ...
    def fit(self, x,
        ...
        x = np.copy(x)
        if augment:
            ax = np.zeros(tuple([rounds * x.shape[0]] + list(x.shape)[1:]), dtype=float)
...
class NumpyArrayIterator(Iterator):
    ...
    def next(self):
        ...
        batch_x = np.zeros(batch_x_shape, dtype=float)
        batch_y = np.zeros(tuple([current_batch_size] + list(self.x.shape)[1:]), dtype=float)
        for i, j in enumerate(index_array):
            x = self.x[j]
            y = self.y[j]
            x, y = self.image_data_generator.random_transform(x.astype(float), y.astype(float))
